...
void position::calculate_bishop_color_key() const{
  ...
  if(pieces(white,pt_bishop)&~dark_squares) key^=0x89A2F3094B57AC47;
  if(pieces(black,pt_bishop)&dark_squares) key^=0xAC4789A2F3094B57;
...
square position::calculate_threat() const{
  ...
  case w_pawn: if(const auto b=
      pawnattack[white][to]&pieces_excluded(black,pt_pawn))
      return lsb(b);
    break;
  case b_pawn: if(const auto b=
      pawnattack[black][to]&pieces_excluded(white,pt_pawn))
      return msb(b);
    break;
  case w_knight: if(const auto b=
      empty_attack[pt_knight][to]&pieces(black,pt_rook,pt_queen))
      return lsb(b);
    break;
  case b_knight: if(const auto b=
      empty_attack[pt_knight][to]&pieces(white,pt_rook,pt_queen))
      return msb(b);
    break;
  case w_bishop: if(const auto b=
      attack_bishop_bb(to,pieces())&pieces(black,pt_rook,pt_queen))
      return lsb(b);
    break;
  case b_bishop: if(const auto b=
      attack_bishop_bb(to,pieces())&pieces(white,pt_rook,pt_queen))
      return msb(b);
    break;
  case w_rook: if(const auto b=attack_rook_bb(to,pieces())&pieces(black,pt_queen)) return lsb(b);
    break;
  case b_rook: if(const auto b=attack_rook_bb(to,pieces())&pieces(white,pt_queen)) return msb(b);
    break;
  case no_piece: break;
  case w_king: break;
  case w_queen: break;
...
void position::init(){
  ...
  for(int castle=no_castle;std::cmp_less_equal(castle,all);++castle){
    ...
    while(b){
      ...
      zobrist::castle[castle]^=k?k:random::rand<uint64_t>();
    }
  }
  zobrist::on_move=random::rand<uint64_t>();
  init_hash_move50(50);
}
...
template<bool yes> void position::do_castle_move(const side me,const square from,const square to,
...
void position::play_move(const uint32_t move,const bool gives_check){
  ...
  if(move_type(move)!=castle_move) relocate_piece(me,piece,from,to);
