...
sfactor endgame_kbpk(const position& pos)
{
	...
	if (const auto white_attack_bb = pos.attack_from<pt_king>(pos.king(strong))
		| pos.attack_from<pt_bishop>(pos.piece_square(strong, pt_bishop)); pos.number(strong, pt_pawn) == 1 && pos.
		on_move() == weak
	...
	if (!(white_pawns & ~file_a_bb)
		...
		&& black_pawns & bb(a7)
		&& king_cover_bb & bb(b8)
		&& (!(black_pawns & file_b_bb) || rank_of(lsb(white_pawns & file_a_bb)) >= rank_of(msb(black_pawns & file_b_bb))))
		return draw_factor;
	...
	if (white_pawns == bb2(a6, b5)
		&& (black_pawns & (bb2(a7, b6)
			| bb3(b7, c7, c6))) == bb2(a7, b6)
		&& king_cover_bb & bb(b8))
		return draw_factor;
	if (white_pawns == bb3(a6, b5, c4)
		&& (black_pawns & (bb3(a7, b6, c5)
			| bb3(b7, c7, c6)
			| bb3(d7, d6, d5))) == bb3(a7, b6, c5)
		&& king_cover_bb & bb(b8))
		return draw_factor;
	return no_factor;
}
...
sfactor endgame_kbpkn(const position& pos)
{
	...
	if (const auto weak_king_sq = pos.king(weak); file_of(weak_king_sq) == file_of(pawn_sq)
		...
		&& (different_color(weak_king_sq, strong_bishop_sq) || relative_rank(strong, weak_king_sq) <= rank_6))
		return draw_factor;
	return no_factor;
}
template <side strong>
sfactor endgame_kbppkb(const position& pos)
{
	...
	case 1:
	{
		if (square_k == block_sq1 && different_color(square_k, wb_sq) && (bb_sq == block_sq2 || pos.attack_from<pt_bishop>(block_sq2)
			& pos.pieces(weak, pt_bishop) || rank_distance(psq1, psq2) >= 2))
			return draw_factor;
...
sfactor endgame_kpk(const position& pos)
{
	...
	if (const auto pawns = pos.pieces(strong, pt_pawn); !(pawns & ~ranks_forward_bb(weak, rank_of(square_k)))
		&& !(pawns & ~file_a_bb && pawns & ~file_h_bb) && file_distance(square_k, lsb(pawns)) <= 1)
		return draw_factor;
	return no_factor;
}
...
sfactor endgame_krkp(const position& pos)
{
		...
		(rank_of(pawn) <= rank_3 || rank_of(black_king) <= rank_of(pawn)))
	{
		...
		if (const bool pawn_attacked = (pos.attack_from<pt_rook>(pos.piece_square(strong, pt_rook))
			| pos.attack_from<pt_king>(pos.king(strong))) & pos.piece_square(weak, pt_pawn); bk_distance <= 1 + (pos.
				on_move() == weak || !pawn_attacked))
		{
