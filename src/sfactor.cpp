...
sfactor endgame_kbpk(const position& pos)
{
	...
	if (const auto white_attack_bb = pos.attack_from<pt_king>(pos.king(strong))| pos.attack_from<pt_bishop>(pos.piece_square(strong, pt_bishop));
		pos.number(strong, pt_pawn) == 1 && pos.on_move() == weak&& pos.pieces(strong, pt_pawn) & black_attack_bb & ~white_attack_bb)return draw_factor;
	...
	if (dark_squares & bishop)
	{
		...
		for (auto i = 0; i < pos.number(strong, pt_pawn); i++)white_pawns |= relative_square(strong, static_cast<square>(pos.piece_list(strong, pt_pawn)[i] ^ 07));
		for (auto i = 0; i < pos.number(weak, pt_pawn); i++)black_pawns |= relative_square(strong, static_cast<square>(pos.piece_list(weak, pt_pawn)[i] ^ 07));
	}
	else
	{
		for (auto i = 0; i < pos.number(strong, pt_pawn); i++)white_pawns |= relative_square(strong, pos.piece_list(strong, pt_pawn)[i]);
		for (auto i = 0; i < pos.number(weak, pt_pawn); i++)black_pawns |= relative_square(strong, pos.piece_list(weak, pt_pawn)[i]);
	}
	...
	if (!(white_pawns & ~file_h_bb))
	{
		...
		{
			if (white_pawns & bb(h5) && (black_pawns & bb2(g7, h6)) == bb2(g7, h6)) {}else return draw_factor;
		}
		if (!(black_pawns & file_g_bb)&& square_distance[g7][black_king] < square_distance[g7][white_king] + 1 - (pos.on_move() == strong)
			&& square_distance[h8][black_king] < 7 - rank_of(msb(white_pawns)) - (pos.on_move() == strong))return static_cast<sfactor>(normal_factor / 4);
		if (!(white_pawns & ~bb2(h2, h3)) && black_pawns & bb(h4))	return static_cast<sfactor>(normal_factor / 4);
	}
	if (pawn == g6 && bishop == h7 && king_cover_bb & bb(h8)) return draw_factor;
	if (!(white_pawns & ~file_a_bb)&& white_pawns & bb(a6)&& black_pawns & bb(a7)&& king_cover_bb & bb(b8)&& (!(black_pawns & file_b_bb)
		|| rank_of(lsb(white_pawns & file_a_bb)) >= rank_of(msb(black_pawns & file_b_bb))))return draw_factor;
	if (!(white_pawns & ~file_g_bb)&& white_pawns & bb(g6)&& black_pawns & bb(g7)&& king_cover_bb & bb2(g8, f8))return draw_factor;
	if (pawn == b6&& black_pawns & bb(b7)&& king_cover_bb & bb2(b8, c8))return draw_factor;
	if (white_pawns == bb2(a6, b5)&& (black_pawns & (bb2(a7, b6)| bb3(b7, c7, c6))) == bb2(a7, b6)&& king_cover_bb & bb(b8))return draw_factor;
	if (white_pawns == bb3(a6, b5, c4)&& (black_pawns & (bb3(a7, b6, c5)| bb3(b7, c7, c6)| bb3(d7, d6, d5))) == bb3(a7, b6, c5)&& king_cover_bb & bb(b8))return draw_factor;
	return no_factor;
}
...
sfactor endgame_kbpkb(const position& pos)
{
	...
	if (weak_side_to_move&& pos.attack_from<pt_bishop>(pos.piece_square(strong, pt_pawn)) & pos.pieces(weak, pt_bishop))return draw_factor;
	if (different_color(pos.piece_square(strong, pt_bishop), pos.piece_square(weak, pt_bishop)))
	{
		...
		const auto white_attack = pos.attack_from<pt_pawn>(pawn, white) | pos.attack_from<pt_bishop>(white_bishop) | pos.attack_from<pt_king>(white_king);
		if (relative_rank(strong, pawn) <= rank_5) return draw_factor;
		if (forward_bb(strong, pawn) & black_king && different_color(black_king, white_bishop))	return draw_factor;
		if (forward_bb(strong, pawn) & (pos.attack_from<pt_bishop>(black_bishop) | black_bishop) && (weak_side_to_move || !(white_attack & black_bishop))) return draw_factor;
	}
	else
	{
		const auto flop = strong == white ? (pos.pieces(white, pt_bishop) & dark_squares ? 0 : 07) : pos.pieces(black, pt_bishop) & ~dark_squares ? 070 : 077;
		...
		if (black_king > pawn&& file_of(pawn) == file_of(black_king)&& ~dark_squares & black_king)return draw_factor;
		if (square_distance[pawn][black_king] == 1&& rank_of(black_king) < rank_of(pawn)&& abs(file_of(black_king) - file_of(white_king)) <= weak_side_to_move
			&& !(averbakh_rule & pawn))	return draw_factor;
	}
	return no_factor;
}
...
sfactor endgame_kbpkn(const position& pos)
{
	...
	if (const auto weak_king_sq = pos.king(weak); file_of(weak_king_sq) == file_of(pawn_sq)
		&& relative_rank(strong, pawn_sq) < relative_rank(strong, weak_king_sq)&& (different_color(weak_king_sq, strong_bishop_sq)
			|| relative_rank(strong, weak_king_sq) <= rank_6))return draw_factor;
	return no_factor;
}
template <side strong>
sfactor endgame_kbppkb(const position& pos)
{
	...
	if (!different_color(wb_sq, bb_sq)) return no_factor;
	...
	case 0:
	{
		if (file_of(square_k) == file_of(block_sq1) && relative_rank(strong, square_k) >= relative_rank(strong, block_sq1) && different_color(square_k, wb_sq)) return draw_factor;
		return no_factor;
	}
	case 1:
	{
		if (square_k == block_sq1 && different_color(square_k, wb_sq) && (bb_sq == block_sq2 || pos.attack_from<pt_bishop>(block_sq2)
			& pos.pieces(weak, pt_bishop) || rank_distance(psq1, psq2) >= 2))	return draw_factor;
		if (square_k == block_sq2 && different_color(square_k, wb_sq) && (bb_sq == block_sq1 || pos.attack_from<pt_bishop>(block_sq1) & pos.pieces(weak, pt_bishop)))
			return draw_factor;
		return no_factor;
	}
	default:
...
sfactor endgame_kpk(const position& pos)
{
	...
	if (const auto pawns = pos.pieces(strong, pt_pawn); !(pawns & ~ranks_forward_bb(weak, rank_of(square_k)))
		&& !(pawns & ~file_a_bb && pawns & ~file_h_bb) && file_distance(square_k, lsb(pawns)) <= 1) 	return draw_factor;
	return no_factor;
}
...
sfactor endgame_kpkp(const position& pos)
{
	...
	if (rank_of(p_sq) >= rank_5 && file_of(p_sq) != file_a)	return no_factor;
...
sfactor endgame_krkp(const position& pos)
{
		...
		(rank_of(pawn) <= rank_3 || rank_of(black_king) <= rank_of(pawn)))
	{
		...
		if (const bool pawn_attacked = (pos.attack_from<pt_rook>(pos.piece_square(strong, pt_rook)) | pos.attack_from<pt_king>(pos.king(strong)))
			& pos.piece_square(weak, pt_pawn); bk_distance <= 1 + (pos.on_move() == weak || !pawn_attacked))
		{
			...
			{
				if (const auto wk_distance2 = square_distance[white_king][pawn] - (pos.on_move() == strong);
					wk_distance2 > bk_distance || rank_of(white_king) - (pos.on_move() == strong) > rank_of(pawn))
					return draw_factor;
			}
		}
	}
...
sfactor endgame_krpkb(const position& pos)
{
	...
	if (pos.pieces(pt_pawn) & (file_a_bb | file_h_bb))
	{
		...
		{
			if (const auto d = distance(psq + 3 * push, square_k); d <= 2 && !(d == 0 && square_k == pos.king(strong) + 2 * push)) return static_cast<sfactor>(38);
...
sfactor endgame_krppkrp(const position& pos)
{
	...
	if (pos.is_passed_pawn(strong, wp_sq1) || pos.is_passed_pawn(strong, wp_sq2)) return no_factor;
	if (const auto r = std::max(relative_rank(strong, wp_sq1), relative_rank(strong, wp_sq2));
		file_distance(bk_sq, wp_sq1) <= 1
...
{
	const auto weak = ~strong;
	if (!(pos.pieces(weak, pt_pawn) & (strong == white ? bb4(a2, c2, f2, h2) : bb4(a7, c7, f7, h7)))) return no_factor;
	auto white_king = relative_square(strong, pos.king(strong));
	auto black_king = relative_square(strong, pos.king(weak));
	...
	if (const auto tempo = !white_on_move || !(attack & static_cast<square>(pawn ^ flop)); dame != pawn - 8 && rank_of(black_king) <= rank_2 + tempo
		&& (square_distance[black_king][pawn] <= 1 + tempo || pawn == c2 && black_king == a1 || pawn == f2 && black_king == h1))
	{
		...
		if (pawn == a2&& (file_of(white_king) > file_e || white_king > d5)) return draw_factor;
		if (pawn == c2&& file_of(black_king) < file_c&& (file_of(white_king) > file_e || white_king > c4)) return draw_factor;
		if (pawn == c2&& file_of(black_king) >= file_c&& (file_of(white_king) > file_g || white_king > d5)) return draw_factor;
		if (!white_on_move&& !bk_check)
		{
			if (pawn == a2&& (square_distance[a1][black_king] == 1|| file_of(dame) != file_a&& !(attack & static_cast<square>(a1 ^ flop))))	return draw_factor;
			if (pawn == c2&& (square_distance[c1][black_king] == 1|| file_of(dame) != file_c&& !(attack & static_cast<square>(c1 ^ flop))))	return draw_factor;
		}
	}
	return no_factor;
}
...
sfactor endgame_kqkrp(const position& pos)
{
	...
	if (const auto rsq = pos.piece_square(weak, pt_rook); relative_rank(weak, king_sq) <= rank_2	&& relative_rank(weak, pos.king(strong)) >= rank_4
		&& relative_rank(weak, rsq) == rank_3&& pos.pieces(weak, pt_pawn)& ~file_a_bb & ~file_h_bb	& pos.attack_from<pt_king>(king_sq)	& pos.attack_from<pt_pawn>(rsq, strong))
		return draw_factor;
