...
namespace movegen {
  ...
  static s_move* all_piece_moves(const position& pos, s_move* moves,
  ...
  template <side me>
  static s_move* generate_pawn_advance(const position& pos, s_move* moves) {
  ...
  s_move* get_castle(const position& pos, s_move* moves) {
    ...
    if (const auto direction = to_k > from_k ? west : east; chess960) {
      ...
      if (const auto from_r = pos.castle_rook_square(to_k);
        attack_rook_bb(to_k, pos.pieces() ^ from_r) &
  ...
  static s_move* get_promotions(const position& pos, s_move* moves, const square to) {
    const auto you = me == white ? black : white;
  ...
  s_move* moves_for_pawn(const position& pos, s_move* moves,
    const uint64_t target) {
    const auto you = me == white ? black : white;
    ...
    if constexpr (mg != captures_promotions) {
      ...
      if constexpr (mg == quiet_checks) {
        ...
        if (const auto discovered_check = pos.info()->x_ray[~pos.on_move()];
          pawns_not_7th_rank & discovered_check) {
          const uint64_t deduction_forward =
            shift_up<me>(pawns_not_7th_rank & discovered_check) &
    ...
    if (pawns_7th_rank && (mg != evade_check || target & eighth_rank)) {
      ...
      uint64_t promotion_right =
        shift_bb<capture_right>(pawns_7th_rank) & your_pieces;
      uint64_t promotion_left =
        shift_bb<capture_left>(pawns_7th_rank) & your_pieces;
    ...
    if constexpr (mg == captures_promotions || mg == evade_check ||
      ...
      uint64_t captureright =
        shift_bb<capture_right>(pawns_not_7th_rank) & your_pieces;
      uint64_t captureleft =
        shift_bb<capture_left>(pawns_not_7th_rank) & your_pieces;
      ...
      if (pos.enpassant_square() != no_square) {
        ...
        captureright = pawns_not_7th_rank &
  ...
  s_move* moves_for_piece(const position& pos, s_move* moves,
    const uint64_t target) {
...
s_move* generate_moves(const position& pos, s_move* moves) {
  assert(mg == captures_promotions || mg == quiet_moves || mg == all_moves ||
    mg == castle_moves);
