...
namespace pawn {
	...
	int eval_pawns(const position& pos, pawn_hash_entry* e)
	{
		...
		const auto center_bind_mask = (file_d_bb | file_e_bb) & (me == white ? rank_5_bb | rank_6_bb | rank_7_bb : rank_4_bb | rank_3_bb | rank_2_bb);
		square sq;
		auto score = 0;
		...
		while ((sq = *p_square++) != no_square)
		{
			...
			bool remaining;
			if (isolated | chain || attackers || my_pawns & pawn_attack_range(you, sq) || relative_rank(me, sq) >= rank_5)	remaining = false;
			else
			{
			...
			{
				e->passed_p[me] |= sq;
				if (chain) score += passed_pawn_values[relative_rank(me, sq)];
			}
			else if (!(stoppers ^ attackers ^ attackers_push) && !double_pawns && popcnt(supported) >= popcnt(attackers) && popcnt(phalanx) >= popcnt(attackers_push))
				score += passed_pawn_values_2[relative_rank(me, sq)];
			if (chain) score += chain_score[closed_file][phalanx != 0][popcnt(supported)][relative_rank(me, sq)];
			else if (isolated)score -= isolated_pawn[closed_file][f];
			else if (remaining)score -= remaining_score[closed_file];
			else score -= un_supported_pawn[closed_file];
			if (double_pawns)score -= doubled_pawn_distance[f][rank_distance(sq, front_square(me, double_pawns))];
			if (attackers)score += pawn_attacker_score[relative_rank(me, sq)];
		}
