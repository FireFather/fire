...
void hash::init(const size_t mb_size){
  const size_t new_size=static_cast<size_t>(1)
    <<msb(mb_size*1024*1024/sizeof(bucket));
  if(new_size==buckets_) return;
  buckets_=new_size;
  bucket_index_mask_=buckets_-1;
  hash_mem_.reset(static_cast<bucket*>(calloc(buckets_*sizeof(bucket)+63,1)));
}

void hash::clear() const{
  for(size_t i=0;i<buckets_;++i) hash_mem_.get()[i]=bucket{};
}

main_hash_entry* hash::probe(const uint64_t key) const{
  auto* const hash_entry=&hash_mem_[key&bucket_index_mask_].entry[0];
...
main_hash_entry* hash::replace(const uint64_t key) const{
  ...
  main_hash_entry* replacement=hash_entry;
...
int hash::hash_full() const{
  constexpr int i_max=999/bucket_size+1;
  int cnt=0;
