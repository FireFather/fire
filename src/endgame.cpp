...
namespace endgame {
	square normalize_pawn_side(const position& pos, const side strong_side, square sq)
	{
		...
		if (file_of(pos.piece_square(strong_side, pt_pawn)) >= file_e)sq = static_cast<square>(sq ^ 7);
		if (strong_side == black)sq = ~sq;
		return sq;
	}
	...
	uint64_t material_key(const side color, const char* pieces)
	{
		...
		for (auto piece = pt_king; piece <= pt_queen; ++piece)
		{
			...
			for (auto cnt = 0; cnt < number; ++cnt)material_key ^= zobrist::psq[make_piece(color, piece)][cnt];
			number = pieces[piece + 8] - '0';
			for (auto cnt = 0; cnt < number; ++cnt)material_key ^= zobrist::psq[make_piece(~color, piece)][cnt];
		}
		return material_key;
	}
	uint64_t attack_king_inc(const square s) { return empty_attack[pt_king][s] | s; }
}
...
int endgame_kqkr(const position& pos)
{
	...
	const auto result = 4 * value_pawn + endgame::push_to_side[weak_k] + endgame::draw_closer[distance(strong_k, weak_k)];
	return strong == pos.on_move() ? result : -result;
}
...
{
	if (pos.is_in_check()) return score_0;
	const auto weak = ~strong;
	if (pos.on_move() == weak && !at_least_one_legal_move(pos)) return draw_score;
	const auto strong_k = pos.king(strong);
	const auto weak_k = pos.king(weak);
	auto result = value_of_material(pos.non_pawn_material(strong)) + pos.number(strong, pt_pawn) * value_pawn + endgame::push_to_side[weak_k] + endgame::draw_closer[distance(strong_k, weak_k)];
	if (result < win_score)
		if (pos.number(strong, pt_queen) + pos.number(strong, pt_rook) || pos.number(strong, pt_bishop) && pos.number(strong, pt_knight)
...
int endgames::probe_scale_factor(const uint64_t key, side& strong_side)
{
	...
	if (iteration != map_scale_factor_.end()) return strong_side = white, iteration->second;
	iteration = map_scale_factor_.find(key ^ black_modifier);
	if (iteration != map_scale_factor_.end()) return strong_side = black, iteration->second;
	return -1;
}
...
inline int value_of_material(const int val) { return 16 * static_cast<int>(val); }
