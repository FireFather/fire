...
enum ptype : uint8_t{
  no_piece=0,w_king=1,w_pawn=2,w_knight=3,w_bishop=4,w_rook=5,w_queen=6,b_king=9,
  b_pawn=10,b_knight=11,b_bishop=12,b_rook=13,b_queen=14,num_pieces=16
};
...
public:
  ...
  template<uint8_t> [[nodiscard]] uint64_t attack_from(square sq) const;
  template<uint8_t> [[nodiscard]] uint64_t attack_from(square sq,side color) const;
...
private:
  ...
  template<bool yes> void do_castle_move(side me,square from,square to,square& from_r,
    square& to_r);
...
template<> inline uint64_t position::attack_from<pt_pawn>(const square sq,
