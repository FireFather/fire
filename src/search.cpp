...
int alpha_beta(position& pos, int alpha, int beta, int depth, bool cut_node) {
  ...
  if (my_thread == thread_pool.main()) {
    if (auto* main_thread = static_cast<mainthread*>(my_thread);
        ++main_thread->interrupt_counter >= 4096) {
      if (main_thread->quick_move_evaluation_busy) {
        if (main_thread->quick_move_evaluation_stopped) return alpha;
        if (auto elapsed = time_control.elapsed();
            elapsed > 1000 || elapsed > time_control.optimum() / 16) {
  ...
  if (!root_node) {
    if (signals.stop_analyzing.load(std::memory_order_relaxed) ||
        pi->move_repetition || pi->ply >= max_ply)
      return pi->ply >= max_ply && !state_check ? evaluate::eval(pos)
                                                : draw[pos.on_move()];
  ...
  hash_value =
      hash_entry ? value_from_hash(hash_entry->value(), pi->ply) : no_score;
  hash_move = root_node    ? my_thread->root_moves[my_thread->active_pv].pv[0]
              : hash_entry ? hash_entry->move()
                           : no_move;

  if (!pv_node && hash_value != no_score && hash_entry->depth() >= depth &&
      (hash_value >= beta ? hash_entry->bounds() & south_border
  ...
  if (hash_entry && hash_entry->eval() != no_score) {
    ...
    if (hash_value != no_score)
      if (hash_entry->bounds() &
          (hash_value > eval ? south_border : north_border))
        eval = hash_value;
  } else {
    ...
    hash_entry->save(key64, no_score, no_limit + pi->strong_threat, no_depth,
                     no_move, pi->position_value, main_hash.age());
  }

  if (pi->previous_move != null_move && (pi - 1)->position_value != no_score &&
    ...
    pos.thread_info()->max_gain_table.update(pi->moved_piece, pi->previous_move,
                                             gain);
  }

  if (pi->no_early_pruning) goto no_early_pruning;

  if (constexpr auto razoring_min_depth = 4;
      !pv_node && depth < razoring_min_depth * plies && !hash_move &&
      eval + razor_margin <= alpha) {
    if (constexpr auto razoring_qs_min_depth = 2;
        depth < razoring_qs_min_depth * plies)
      return q_search<nonPV, false>(pos, alpha, beta, depth_0);

    auto r_alpha = alpha - razor_margin;
    if (auto val =
            q_search<nonPV, false>(pos, r_alpha, r_alpha + score_1, depth_0);
        val <= r_alpha)
      return val;
  }

  if (constexpr auto futility_min_depth = 7;
      !root_node && depth < futility_min_depth * plies &&
      eval - futility_margin(depth) >= beta && eval < win_score &&
      pi->non_pawn_material[pos.on_move()])
    return eval - futility_margin(depth);

  if (constexpr auto null_move_min_depth = 2;
      !pv_node && depth >= null_move_min_depth * plies &&
      eval >= beta + null_move_tempo_mult &&
      (!pi->strong_threat || depth >= null_move_strong_threat_mult * plies) &&
      (pi->position_value >= beta ||
       depth >= null_move_pos_val_less_than_beta_mult * plies) &&
      pi->non_pawn_material[pos.on_move()] &&
    ...
    auto R = no_depth;
    R = depth < 4 * plies ? depth : (null_move_tm_base +
               null_move_tm_mult * (depth / plies) +
               std::max(std::min(null_move_depth_greater_than_mult * (eval - beta) /
               null_move_depth_greater_than_div -
               null_move_depth_greater_than_sub -
               null_move_depth_greater_than_cut_node_mult *
               cut_node -
               null_move_depth_greater_than_cut_node_mult *
               static_cast<int>((hash_move != no_move)),
               3 * 256),0)) / 256 * plies;
    ...
    auto value =
        depth - R < plies
            ...
            : -alpha_beta<nonPV>(pos, -beta, -beta + score_1, depth - R,
                                 !cut_node);
    (pi + 1)->no_early_pruning = false;
    pos.take_null_back();

    if (constexpr auto value_less_than_beta_margin = 100;
        (pi - 1)->lmr_reduction &&
    ...
    if (value >= beta) {
      ...
      if (constexpr auto value_greater_than_beta_max_depth = 12;
          depth < value_greater_than_beta_max_depth * plies &&
          abs(beta) < win_score)
        return value;
  ...
  } else if (constexpr auto dummy_null_move_threat_min_depth_mult = 6;
             thread_pool.dummy_null_move_threat &&
             depth >= dummy_null_move_threat_min_depth_mult * plies &&
             eval >= beta && (pi - 1)->lmr_reduction) {
    pi->mp_end_list = (pi - 1)->mp_end_list;
    pos.play_null_move();
    (pi + 1)->no_early_pruning = true;
    auto null_beta = beta - 240;
    auto value = -alpha_beta<nonPV>(pos, -null_beta, -null_beta + score_1,
                                    depth / 2 - 2 * plies, false);
    (pi + 1)->no_early_pruning = false;
    pos.take_null_back();
...
no_early_pruning:
  ...
  if (constexpr auto no_early_pruning_min_depth = 5;
      !pv_node && depth >= no_early_pruning_min_depth * plies &&
    ...
    while ((move = movepick::pick_move(pos)) != no_move) {
      if (pos.legal_move(move)) {
        pos.play_move(move);
        auto value = -alpha_beta<nonPV>(pos, -pc_beta, -pc_beta + score_1,
                                        pc_depth, !cut_node);
  ...
  if (constexpr auto no_early_pruning_pv_node_depth_min = 5;
      ...
      (pv_node || cut_node ||
       pi->position_value +
               static_cast<int>(no_early_pruning_position_value_margin) >=
           beta)) {
    auto d =
        depth - 2 * plies -
        (pv_node ? depth_0 : static_cast<uint32_t>(depth) / plies / 4 * plies);
    pi->no_early_pruning = true;
...
view_all_moves:
  ...
  auto only_quiet_check_moves =
      !root_node && depth < only_quiet_check_moves_max_depth * plies &&
  ...
  auto late_move_count = depth < late_move_count_max_depth * plies
                             ? late_move_number(depth, progress)
                             : 999;
  ...
  while ((move = movepick::pick_move(pos)) != no_move) {
    ...
    if (!bench_active && root_node && my_thread == thread_pool.main()) {
      if (constexpr auto info_currmove_interval = 4000;
    ...
    if (constexpr auto excluded_move_min_depth = 8;
        !root_node && move == hash_move &&
        depth >= excluded_move_min_depth * plies &&
        hash_entry->bounds() & south_border &&
        hash_entry->depth() >=
            depth - excluded_move_hash_depth_reduction * plies &&
        extension < plies && abs(hash_value) < win_score &&
        pos.legal_move(move)) {
      ...
      auto r_beta =
          hash_value - depth / plies *
                           excluded_move_r_beta_hash_value_margin_mult /
                           excluded_move_r_beta_hash_value_margin_div;
      auto r_depth = depth / plies / 2 * plies;
      pi->excluded_move = move;

      if (auto value = alpha_beta<nonPV>(pos, r_beta - score_1, r_beta, r_depth,
                                         !pv_node && cut_node);
          value < r_beta)
        extension = plies;
    ...
    if (!(root_node | capture_or_promotion | gives_check) &&
        best_score > -longest_mate_score && !pos.advanced_pawn(move) &&
        pi->non_pawn_material[pos.on_move()]) {
      ...
      if (constexpr auto quiet_moves_max_depth = 6;
          depth < quiet_moves_max_depth * plies && pi->mp_stage >= quietmoves) {
        auto offset =
      ...
      predicted_depth = std::max(
          new_depth - lmr_reduction(pv_node, progress, depth, move_number),
          depth_0);
      ...
      if (constexpr auto predicted_depth_see_test_base = 300;
          predicted_depth < predicted_depth_max_depth * plies &&
          !pos.see_test(
              move,
              std::min(see_0, predicted_depth_see_test_base -
                                  predicted_depth_see_test_mult *
                                      predicted_depth * predicted_depth / 64)))
        continue;
    }

    else if (constexpr auto non_root_node_max_depth = 7;
             !root_node && depth < non_root_node_max_depth * plies &&
      ...
      if (constexpr auto non_root_node_see_test_base = 150;
          pi->mp_stage != good_captures && extension != plies &&
          !pos.see_test(move, std::min(see_knight - see_bishop,
                                       non_root_node_see_test_base -
                                           non_root_node_see_test_mult * depth *
                                               depth / 64)))
        continue;
    }
    ...
    if (constexpr auto lmr_min_depth = 3; depth >= lmr_min_depth * plies &&
      ...
      auto offset =
          move_value_stats::calculate_offset(moved_piece, to_square(move));
      auto stats_value =
          static_cast<int>(
              state_check
      ...
      if ((pi - 1)->stats_value != sort_max)
        r -= std::min(plies,
                      std::max(-plies, (stats_value - (pi - 1)->stats_value) /
                                           4096 * (plies / 8)));
      ...
      if (constexpr auto lmr_reduction_min = 5;
          value > alpha && pi->lmr_reduction >= lmr_reduction_min * plies) {
        pi->lmr_reduction = static_cast<uint8_t>(lmr_reduction_min * plies / 2);
        value =
            -alpha_beta<nonPV>(pos, -(alpha + score_1), -alpha,
                               new_depth - lmr_reduction_min * plies / 2, true);
      }
    ...
    if (full_search_needed) {
      value = new_depth < plies
                  ? gives_check ? -q_search<nonPV, true>(
                                      pos, -(alpha + score_1), -alpha, depth_0)
                                : -q_search<nonPV, false>(
                                      pos, -(alpha + score_1), -alpha, depth_0)
                  : -alpha_beta<nonPV>(pos, -(alpha + score_1), -alpha,
    ...
    if (my_thread == thread_pool.main() &&
        static_cast<mainthread*>(my_thread)->quick_move_evaluation_stopped)
      return alpha;

    if (root_node) {
      auto move_index = my_thread->root_moves.find(move);

      if (auto& root_move = my_thread->root_moves.moves[move_index];
          move_number == 1 || value > alpha) {
    ...
    if (value > best_score) {
      ...
      if (value > alpha) {
        if (pv_node && my_thread == thread_pool.main() &&
    ...
    if (!capture_or_promotion && move != best_move &&
        quiet_move_number < max_quiet_moves)
      quiet_moves[quiet_move_number++] = move;
  }

  if (best_score == -max_score)
    best_score = pi->excluded_move ? alpha
                 : state_check     ? gets_mated(pi->ply)
                                   : draw[pos.on_move()];

  else if (best_move) {
    update_stats(pos, state_check, best_move, depth, quiet_moves,
                 quiet_move_number);
  }
    ...
    if (constexpr auto fail_low_counter_move_bonus_min_depth_mult = 3;
        ...
        pi->position_value >=
            alpha -
                static_cast<int>(fail_low_counter_move_bonus_min_depth_margin))
      update_stats_quiet(pos, state_check, depth, quiet_moves,
                         quiet_move_number);

    if (constexpr auto fail_low_counter_move_bonus_min_depth = 3;
        depth >= fail_low_counter_move_bonus_min_depth * plies &&
        !state_check && !pi->captured_piece && pi->move_counter_values) {
      if (constexpr auto counter_move_bonus_min_depth = 18;
          depth < counter_move_bonus_min_depth * plies) {
        auto bonus = counter_move_value(depth);
        auto offset = counter_move_values::calculate_offset(
  ...
  if (!pi->excluded_move) {
    ...
    hash_entry->save(key64, value_to_hash(best_score, pi->ply),
                     (best_score >= beta     ? south_border
                      : pv_node && best_move ? exact_value
                                             : north_border) +
                         pi->strong_threat,
...
void copy_pv(uint32_t* pv, const uint32_t move, const uint32_t* pv_lower) {
  *pv++ = move;
  if (pv_lower)
    while (*pv_lower != no_move) *pv++ = *pv_lower++;
  *pv = no_move;
}
...
void init() {
  ...
  for (int d = plies; d < 64 * plies; ++d)
    for (auto n = 2; n < 64; ++n) {
      const auto rr = log(static_cast<double>(d) / plies) * log(n) / 2 *
                      static_cast<int>(plies);
      if (rr < 6.4) continue;
      const auto r = static_cast<int>(std::lround(rr));

      lm_reductions[false][1][d][n] = static_cast<uint8_t>(r);
      lm_reductions[false][0][d][n] =
          static_cast<uint8_t>(r + (r < 2 * plies ? 0 * plies : plies));
      lm_reductions[true][1][d][n] =
          static_cast<uint8_t>(std::max(r - plies, depth_0));
      lm_reductions[true][0][d][n] = lm_reductions[true][1][d][n];
    }

  for (auto d = 1; d < max_ply; ++d) {
    constexpr auto counter_move_bonus_value = 24;
    counter_move_bonus[d] = static_cast<int>(
...
int q_search(position& pos, int alpha, const int beta, const int depth) {
  ...
  constexpr int q_search_futility_value[num_pieces] = {
      qs_futility_value_0, qs_futility_value_1, qs_futility_value_2,
      qs_futility_value_3, qs_futility_value_4, qs_futility_value_5,
      qs_futility_value_6, qs_futility_value_7, qs_futility_value_0,
      qs_futility_value_1, qs_futility_value_2, qs_futility_value_3,
      qs_futility_value_4, qs_futility_value_4, qs_futility_value_6,
      qs_futility_value_7};
  ...
  if (!pv_node && hash_value != no_score && hash_entry->depth() >= hash_depth &&
      (hash_value >= beta ? hash_entry->bounds() & south_border
                          : hash_entry->bounds() & north_border)) {
    return hash_value;
  }
  ...
  if (state_check) {
    ...
    if (hash_entry && hash_entry->eval() != no_score) {
      ...
      if (hash_value != no_score)
        if (hash_entry->bounds() &
            (hash_value > best_value ? south_border : north_border))
      ...
      if (best_value >= beta) {
        ...
        hash_entry->save(key64, value_to_hash(best_value, pi->ply),
                         south_border + pi->strong_threat, no_depth, no_move,
  ...
  while ((move = movepick::pick_move(pos)) != no_move) {
    ...
    bool gives_check =
        move < static_cast<uint32_t>(castle_move) &&
                !pos.discovered_check_possible()
            ? pi->check_squares[piece_type(pos.moved_piece(move))] &
                  to_square(move)
            : pos.give_check(move);

    if (!state_check && !gives_check && futility_basis > -win_score &&
      ...
      futility_value +=
          pos.thread_info()->capture_history[capture][to_square(move)] /
          qs_futility_value_capture_history_add_div;
      ...
      if (constexpr auto qs_futility_basis_margin = 102;
          futility_basis + static_cast<int>(qs_futility_basis_margin) <=
          alpha) {
    ...
    if (state_check) {
      if (best_value > -longest_mate_score && !pos.is_capture_move(move) &&
          move < static_cast<uint32_t>(promotion_p)) {
        const auto moved_piece = pos.moved_piece(move);

        if (!gives_check) {
          const auto offset = counter_move_values::calculate_offset(
              moved_piece, to_square(move));

          if (const auto stats_value =
                  static_cast<int>(
                      pos.thread_info()->evasion_history.value_at_offset(
                          offset)) +
                  (pi->move_counter_values
                       ? static_cast<int>(
                             pi->move_counter_values->value_at_offset(offset))
                       : sort_zero) +
                  ((pi - 1)->move_counter_values
                       ? static_cast<int>(
                             (pi - 1)->move_counter_values->value_at_offset(
                                 offset))
                       : sort_zero) +
                  ((pi - 3)->move_counter_values
                       ? static_cast<int>(
                             (pi - 3)->move_counter_values->value_at_offset(
                                 offset))
                       : sort_zero);
              stats_value < static_cast<int>(qs_stats_value_sortvalue))
            continue;
        }
  ...
  skip_see_test:
    ...
    pos.play_move(move, gives_check);
    const int value =
        gives_check ? -q_search<nt, true>(pos, -beta, -alpha, depth - plies)
                    : -q_search<nt, false>(pos, -beta, -alpha, depth - plies);
    pos.take_move_back(move);

    if ((pi + 1)->captured_piece) {
      constexpr auto
          qs_skip_see_test_value_less_than_equal_to_alpha_sort_value = 2000;
      constexpr auto qs_skip_see_test_value_greater_than_alpha_sort_value =
          1000;
      if (const auto offset = move_value_stats::calculate_offset(
              (pi + 1)->captured_piece, to_square(move));
          value > alpha)
        pos.thread_info()->capture_history.update_plus(
            offset, qs_skip_see_test_value_greater_than_alpha_sort_value);
      else
        pos.thread_info()->capture_history.update_minus(
            offset, qs_skip_see_test_value_less_than_equal_to_alpha_sort_value);
    }
    ...
    if (value > best_value) {
      ...
      if (value > alpha) {
        ...
        if (pv_node && value < beta) {
          ...
          hash_entry = main_hash.replace(key64);
          hash_entry->save(key64, value_to_hash(value, pi->ply),
                           south_border + pi->strong_threat, hash_depth, move,
                           pi->position_value, main_hash.age());
  ...
  hash_entry = main_hash.replace(key64);
  hash_entry->save(
      key64, value_to_hash(best_value, pi->ply),
      (pv_node && best_value > orig_alpha ? exact_value : north_border) +
          pi->strong_threat,
...
void update_stats(const position& pos, const bool state_check,
                  ...
                  const uint32_t* quiet_moves, const int quiet_number) {
  auto* pi = pos.info();
  ...
  if (!pos.capture_or_promotion(move)) {
    ...
    if (cmh)
      ti->counter_moves.update(pi->moved_piece, to_square(pi->previous_move),
                               static_cast<unsigned short>(move));

    if (cmh && fmh)
      ti->counter_followup_moves.update(
          (pi - 1)->moved_piece, to_square((pi - 1)->previous_move),
          pi->moved_piece, to_square(pi->previous_move), move);

    if (constexpr auto update_stats_max_depth = 18;
        depth < update_stats_max_depth * plies) {
      const auto bonus = counter_move_value(depth);
      const auto hist_bonus = history_bonus(depth);
      auto offset = move_value_stats::calculate_offset(pos.moved_piece(move),
                                                       to_square(move));
      ...
      for (auto i = 0; i < quiet_number; ++i) {
        offset = move_value_stats::calculate_offset(
            pos.moved_piece(quiet_moves[i]), to_square(quiet_moves[i]));
  ...
  if ((pi - 1)->move_number == 1 && !pi->captured_piece) {
    if (constexpr auto update_stats_move_1_max_depth = 18;
        depth < update_stats_move_1_max_depth * plies) {
...
void update_stats_minus(const position& pos, const bool state_check,
  ...
  if (!pos.capture_or_promotion(move)) {
    if (constexpr auto update_stats_minus_max_depth = 18;
        depth < update_stats_minus_max_depth * plies) {
      const auto bonus = counter_move_value(depth);
      const auto hist_bonus = history_bonus(depth);
      const auto offset = move_value_stats::calculate_offset(
          pos.moved_piece(move), to_square(move));
      history.update_minus(offset, hist_bonus);
      if (cmh) cmh->update_minus(offset, bonus);
...
void update_stats_quiet(const position& pos, const bool state_check,
  ...
  if (constexpr auto update_stats_quiet_max_depth = 18;
      depth < update_stats_quiet_max_depth * plies) {
...
void mainthread::begin_search() {
  ...
  thread_pool.fifty_move_distance =
      std::min(50, std::max(thread_pool.fifty_move_distance,
  ...
  if (thread_pool.piece_contempt) {
    ...
    if (const auto v2 = evaluate::eval(*root_position);
        abs(v1) < win_score && abs(v2) < win_score)
  ...
  if (thread_pool.analysis_mode) {
    ...
    search::draw[me] = draw_score - default_draw_value *
                                        root_position->game_phase() /
                                        middlegame_phase;
    search::draw[~me] = draw_score + default_draw_value *
                                         root_position->game_phase() /
                                         middlegame_phase;
  ...
  for (const auto& move : legal_move_list(*root_position))
    if (search::param.search_moves.empty() ||
        search::param.search_moves.find(move) >= 0)
  ...
  if (root_moves.move_number == 0) {
    ...
    root_moves[0].score =
        root_position->is_in_check() ? -mate_score : draw_score;
    root_moves[0].depth = main_thread_inc;
    thread_pool.active_thread_count = 1;
  } else {
    thread_pool.multi_pv_max =
        std::min(thread_pool.multi_pv_max, root_moves.move_number);
    thread_pool.multi_pv =
        std::min(thread_pool.multi_pv, root_moves.move_number);
  ...
  if (!search::signals.stop_analyzing &&
      (search::param.ponder || search::param.infinite)) {
  ...
  if (!this->quick_move_played && thread_pool.multi_pv == 1 &&
      !search::param.depth && root_moves[0].pv[0] != no_move) {
    for (auto i = 1; i < thread_pool.active_thread_count; ++i) {
      if (auto* th = thread_pool.threads[i];
...
void thread::begin_search() {
  ...
  if (!main_thread) {
    root_position->copy_position(thread_pool.root_position, this,
                                 thread_pool.root_position_info);
  ...
  (pi - 1)->move_number = 0;
  (pi - 4)->move_counter_values = (pi - 3)->move_counter_values =
      (pi - 2)->move_counter_values = (pi - 1)->move_counter_values =
          pi->move_counter_values = nullptr;
  ...
  if (main_thread) {
    ...
    main_thread->quick_move_evaluation_busy =
        main_thread->quick_move_evaluation_stopped = false;
    main_thread->best_move_changed = 0;
    for (auto i = 1; i <= max_ply; i++) (pi + i)->pawn_key = 0;
  }

  if (main_thread && !search::param.ponder && !thread_pool.analysis_mode &&
      main_thread->quick_move_allow &&
      main_thread->previous_root_depth >= 12 * plies &&
      thread_pool.multi_pv == 1) {
    if (const auto* hash_entry = main_hash.probe(root_position->key());
        hash_entry && hash_entry->bounds() == exact_value) {
      const auto hash_value =
          search::value_from_hash(hash_entry->value(), pi->ply);
      const auto hash_move = hash_entry->move();

      if (const auto hash_depth = hash_entry->depth();
          hash_depth >= main_thread->previous_root_depth - 3 * plies &&
          hash_move && root_position->legal_move(hash_move) &&
          abs(hash_value) < win_score) {
        constexpr auto v_singular_margin = 102;
        const auto depth_singular =
            std::max(main_thread->previous_root_depth / 2,
                     main_thread->previous_root_depth - 8 * plies);
        const auto v_singular =
            hash_value - static_cast<int>(v_singular_margin);
        pi->excluded_move = hash_move;
        pi->position_value = evaluate::eval(*root_position);
        main_thread->quick_move_evaluation_busy = true;
        const auto val = search::alpha_beta<search::nonPV>(
            *root_position, v_singular - score_1, v_singular, depth_singular,
            false);
  ...
  while (++search_iteration < 100) {
    ...
    for (active_pv = 0;
         active_pv < thread_pool.multi_pv && !search::signals.stop_analyzing;
         ++active_pv) {
      ...
      if (root_depth >= 5 * plies) {
        ...
        alpha = std::max(root_moves[active_pv].previous_score - delta_alpha,
                         -max_score);
        beta = std::min(root_moves[active_pv].previous_score + delta_beta,
                        max_score);
      }
      ...
      while (true) {
        ...
        best_value = search::alpha_beta<search::PV>(*root_position, alpha, beta,
                                                    root_depth, false);

        std::stable_sort(root_moves.moves + active_pv,
                         root_moves.moves + root_moves.move_number);

        if (search::signals.stop_analyzing) break;

        bool fail_high_resolve = main_thread;

        if (constexpr auto time_control_optimum_mult_1 = 124;
            main_thread && best_value >= beta &&
            root_moves[active_pv].pv[0] == prev_best_move &&
            !thread_pool.analysis_mode &&
            time_control.elapsed() >
                time_control.optimum() * time_control_optimum_mult_1 / 1024) {
          if (const auto play_easy_move = root_moves[0].pv[0] == fast_move &&
                                          main_thread->best_move_changed < 31;
              play_easy_move)
            fail_high_resolve = false;

          else if (constexpr auto time_control_optimum_mult_2 = 420;
                   time_control.elapsed() > time_control.optimum() *
                                                time_control_optimum_mult_2 /
                                                1024) {
            const auto improvement_factor = std::max(
                420,
                std::min(
                    improvement_factor_min_base,
                    improvement_factor_max_base +
                        improvement_factor_max_mult * main_thread->failed_low -
                        improvement_factor_bv_mult *
                            (best_value - main_thread->previous_root_score)));
            if (const auto unstable_factor =
                    1024 + main_thread->best_move_changed;
                time_control.elapsed() > time_control.optimum() *
                                             unstable_factor / 1024 *
        ...
        if (best_value <= alpha) {
          ...
          if (main_thread) {
            ...
            search::signals.stop_if_ponder_hit = false;
                   ...
                   best_value >= beta &&
                   (fail_high_resolve ||
                    best_value >= value_pawn * best_value_vp_mult)) {
          alpha = (alpha + beta) / 2;
        ...
        delta_alpha +=
            delta_alpha / 4 + static_cast<int>(delta_alphabeta_value_add);
        delta_beta +=
            delta_beta / 4 + static_cast<int>(delta_alphabeta_value_add);
    ...
    if (!search::signals.stop_analyzing) completed_depth = root_depth;

    if (!main_thread) continue;

    if (search::param.mate && best_value >= longest_mate_score &&
        mate_score - best_value <= 2 * search::param.mate)
      search::signals.stop_analyzing = true;

    if (!thread_pool.analysis_mode && !search::param.ponder &&
        best_value > mate_score - 32 &&
        root_depth >=
            (mate_score - best_value + root_depth_mate_value_bv_add) * plies)
      search::signals.stop_analyzing = true;
    ...
    if (!thread_pool.analysis_mode) {
      if (!search::signals.stop_analyzing &&
        ...
        const auto improvement_factor = std::max(
            420,
            std::min(improvement_factor_min_base,
                     improvement_factor_max_base +
                         improvement_factor_max_mult * main_thread->failed_low -
                         improvement_factor_bv_mult *
                             (best_value - main_thread->previous_root_score)));
        ...
        if (const auto play_easy_move =
                ...
                main_thread->best_move_changed < 31 &&
                time_control.elapsed() > time_control.optimum() * 124 / 1024;
            root_moves.move_number == 1 && search_iteration > 10 ||
...
bool rootmove::ponder_move_from_hash(position& pos) {
  ...
  if (const auto* hash_entry = main_hash.probe(pos.key() ^ pos.draw50_key());
      hash_entry) {
    if (const auto move = hash_entry->move();
...
std::string print_pv(const position& pos, const int alpha, const int beta,
                     const int active_pv, const int active_move) {
  std::stringstream ss;
  ...
  for (auto i = 0; i < multi_pv; ++i) {
    ...
    ss << "info time " << elapsed << " nodes " << visited_nodes << " nps "
       << visited_nodes / elapsed * 1000;

    if (hash_full) ss << " hashfull " << hash_full;

    ss << " depth " << iterate << " seldepth " << sel_depth << " multipv "
       << i + 1 << " score " << score_cp(score);

    if (i == active_pv)
      ss << (score >= beta    ? " lowerbound"
    ...
    for (auto n = 0; n < pv_length; n++)
      ss << " " << move_to_string(root_move.pv[n], pos);
  }
...
void rootmove::pv_from_hash(position& pos) {
  ...
  while (true) {
    ...
    const auto* const hash_entry =
        main_hash.probe(pos.key() ^ pos.draw50_key());
    if (!hash_entry) break;
...
std::string score_cp(int score) {
  ...
  if (abs(score) < longest_mate_score) {
    if (abs(score) < win_score) score -= thread_pool.root_contempt_value;
    ss << "cp " << score / 3;
  } else
    ss << "mate "
       << (score > 0 ? mate_score - score + 1 : -mate_score - score) / 2;
