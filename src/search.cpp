 ...
 */
  ...
  template<nodetype nt> int alpha_beta(position& pos,int alpha,int beta,int depth,bool cut_node){
    ...
    if(constexpr auto razoring_min_depth=4;!pv_node&&depth<razoring_min_depth*plies&&!hash_move&&
      eval+razor_margin<=alpha){
      if(constexpr auto razoring_qs_min_depth=2;depth<razoring_qs_min_depth*plies) return q_search<non_pv,false>(pos,alpha,beta,depth_0);
      auto r_alpha=alpha-razor_margin;
      if(auto val=
        q_search<non_pv,false>(pos,r_alpha,r_alpha+score_1,depth_0);val<=r_alpha)
        return val;
    }
    if(constexpr auto futility_min_depth=7;!root_node&&depth<futility_min_depth*plies&&
      eval-futility_margin(depth)>=beta&&eval<win_score&&
      pi->non_pawn_material[pos.on_move()])
      return eval-futility_margin(depth);
    if(constexpr auto null_move_min_depth=2;!pv_node&&depth>=null_move_min_depth*plies&&
      ...
      auto value=
        depth-R<plies
        ?-q_search<non_pv,false>(pos,-beta,-beta+score_1,depth_0)
        :-alpha_beta<non_pv>(pos,-beta,-beta+score_1,depth-R,
          !cut_node);
      ...
      if(value>=beta){
        ...
        if(constexpr auto value_greater_than_beta_max_depth=12;depth<value_greater_than_beta_max_depth*plies&&
          abs(beta)<win_score)
          return value;
        pi->no_early_pruning=true;
        auto val=
          depth-R<plies
          ?q_search<non_pv,false>(pos,beta-score_1,beta,depth_0)
          :alpha_beta<non_pv>(pos,beta-score_1,beta,depth-R,false);
        pi->no_early_pruning=false;
        if(val>=beta) return value;
      }
    } else if(constexpr auto dummy_null_move_threat_min_depth_mult=6;thread_pool.dummy_null_move_threat&&
      ...
      auto value=-alpha_beta<non_pv>(pos,-null_beta,-null_beta+score_1,
...
no_early_pruning: if(constexpr auto no_early_pruning_min_depth=5;!pv_node&&depth>=no_early_pruning_min_depth*plies&&
      ...
      while((move=movepick::pick_move(pos))!=no_move){
        if(pos.legal_move(move)){
          pos.play_move(move);
          auto value=-alpha_beta<non_pv>(pos,-pc_beta,-pc_beta+score_1,
            pc_depth,!cut_node);
    ...
    if(constexpr auto no_early_pruning_pv_node_depth_min=5;depth>=(pv_node
        ?no_early_pruning_pv_node_depth_min*plies
...
view_all_moves: const counter_move_values* cmh=pi->move_counter_values;
    ...
    while((move=movepick::pick_move(pos))!=no_move){
      ...
      if(gives_check&&
        ...
        extension=plies;
      if(constexpr auto excluded_move_min_depth=8;!root_node&&move==hash_move&&
        ...
        if(auto value=alpha_beta<non_pv>(pos,r_beta-score_1,r_beta,r_depth,
          !pv_node&&cut_node);value<r_beta)
          extension=plies;
      ...
      if(!(root_node|capture_or_promotion|gives_check)&&
        ...
        if(constexpr auto predicted_depth_see_test_base=300;predicted_depth<predicted_depth_max_depth*plies&&
          ...
          continue;
      } else if(constexpr auto non_root_node_max_depth=7;!root_node&&depth<non_root_node_max_depth*plies&&
      ...
      if(constexpr auto lmr_min_depth=3;depth>=lmr_min_depth*plies&&
        ...
        value=-alpha_beta<non_pv>(pos,-(alpha+score_1),-alpha,d,true);
        if(constexpr auto lmr_reduction_min=5;value>alpha&&pi->lmr_reduction>=lmr_reduction_min*plies){
          ...
          value=
            -alpha_beta<non_pv>(pos,-(alpha+score_1),-alpha,
      ...
      if(full_search_needed){
        value=new_depth<plies
          ?gives_check
          ?-q_search<non_pv,true>(
            pos,-(alpha+score_1),-alpha,depth_0)
          :-q_search<non_pv,false>(
            pos,-(alpha+score_1),-alpha,depth_0)
          :-alpha_beta<non_pv>(pos,-(alpha+score_1),-alpha,
      ...
      if(pv_node&&
        ...
        value=new_depth<plies
          ?gives_check
          ?-q_search<is_pv,true>(pos,-beta,-alpha,depth_0)
          :-q_search<is_pv,false>(pos,-beta,-alpha,depth_0)
          :-alpha_beta<is_pv>(pos,-beta,-alpha,new_depth,false);
      }
      pos.take_move_back(move);
      if(signals.stop_analyzing.load(std::memory_order_relaxed)) return alpha;
      if(my_thread==thread_pool.main()&&
        dynamic_cast<mainthread*>(my_thread)->quick_move_evaluation_stopped)
        return alpha;
      if(root_node){
        ...
        if(auto& root_move=my_thread->root_moves.moves[move_index];move_number==1||value>alpha){
          ...
          if(move_number>1&&my_thread==thread_pool.main()) dynamic_cast<mainthread*>(my_thread)->best_move_changed+=1024;
          if(!bench_active&&my_thread==thread_pool.main())
            acout()<<print_pv(pos,alpha,beta,my_thread->active_pv,
                move_index)
              <<std::endl;
        } else root_move.score=-max_score;
      }
      if(value>best_score){
        best_score=value;
        if(value>alpha){
    ...
    if(best_score==-max_score)
      ...
      if(constexpr auto fail_low_counter_move_bonus_min_depth_mult=3;depth>=fail_low_counter_move_bonus_min_depth_mult*plies&&
        pi->position_value>=
        alpha-
  ...
  template<nodetype nt,bool state_check> int q_search(position& pos,int alpha,const int beta,const int depth){
    ...
    while((move=movepick::pick_move(pos))!=no_move){
      ...
      if(!state_check&&!gives_check&&futility_basis>-win_score&&
        ...
        if(constexpr auto qs_futility_basis_margin=102;futility_basis+static_cast<int>(qs_futility_basis_margin)<=
          alpha){
              ...
              ((pi-3)->move_counter_values
                ?static_cast<int>(
                  (pi-3)->move_counter_values->value_at_offset(
                    offset))
                :sort_zero);stats_value<static_cast<int>(qs_stats_value_sortvalue))
              continue;
          }
...
skip_see_test: if(!pos.legal_move(move)) continue;
      pos.play_move(move,gives_check);
      const int value=
        gives_check
      ...
      if((pi+1)->captured_piece){
        ...
        if(const auto offset=move_value_stats::calculate_offset(
          (pi+1)->captured_piece,to_square(move));value>alpha)
    ...
    if((pi-1)->move_number==1&&!pi->captured_piece){
      if(constexpr auto update_stats_move_1_max_depth=18;depth<update_stats_move_1_max_depth*plies){
        ...
        const auto offset=move_value_stats::calculate_offset(
          pi->moved_piece,to_square(pi->previous_move));
        if((pi-1)->move_counter_values) (pi-1)->move_counter_values->update_minus(offset,bonus);
        if((pi-2)->move_counter_values) (pi-2)->move_counter_values->update_minus(offset,bonus);
        if((pi-4)->move_counter_values) (pi-4)->move_counter_values->update_minus(offset,bonus);
      }
    }
  }
...
void mainthread::begin_search(){
  ...
  if(root_moves.move_number==0){
    ...
    for(auto i=1;i<thread_pool.active_thread_count;++i) thread_pool.threads[i]->wake(true);
    thread::begin_search();
  }
  ...
  if(best_thread!=this) best_thread->root_moves[0].depth=root_moves[0].depth;
  if(!bench_active){
...
void thread::begin_search(){
  ...
  if(main_thread&&!search::param.ponder&&!thread_pool.analysis_mode&&
    ...
    if(const auto* hash_entry=main_hash.probe(root_position->key());hash_entry&&hash_entry->bounds()==exact_value){
      ...
      if(const auto hash_depth=hash_entry->depth();hash_depth>=main_thread->previous_root_depth-3*plies&&
        ...
        const auto val=search::alpha_beta<search::non_pv>(
  ...
  while(++search_iteration<100){
    ...
    if(constexpr auto info_depth_interval=1000;!bench_active&&main_thread&&
    ...
    for(active_pv=0;
      ...
      if(root_depth>=5*plies){
        ...
        delta_alpha=delta_alpha_margin+(thread_index&7);
        delta_beta=delta_beta_margin-(thread_index&7);
      ...
      while(true){
        ...
        best_value=search::alpha_beta<search::is_pv>(*root_position,alpha,beta,
          root_depth,false);
        ...
        if(constexpr auto time_control_optimum_mult_1=124;main_thread&&best_value>=beta&&
          ...
          if(const auto play_easy_move=root_moves[0].pv[0]==fast_move&&
            main_thread->best_move_changed<31;play_easy_move)
            fail_high_resolve=false;
          else if(constexpr auto time_control_optimum_mult_2=420;time_control.elapsed()>time_control.optimum()*
            ...
            if(const auto unstable_factor=
                1024+main_thread->best_move_changed;time_control.elapsed()>time_control.optimum()*
        ...
        } else if(constexpr auto best_value_vp_mult=8;best_value>=beta&&
          (fail_high_resolve||
...
std::string print_pv(const position& pos,const int alpha,const int beta,
  ...
  for(auto i=0;i<multi_pv;++i){
    ...
    for(sel_depth=0;sel_depth<max_ply;sel_depth++) if((pi+sel_depth)->pawn_key==0) break;
    ...
    for(auto n=0;n<pv_length;n++) ss<<" "<<move_to_string(root_move.pv[n],pos);
  }
  return ss.str();
}
